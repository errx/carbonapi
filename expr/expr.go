...
func EvalExpr(e *expr, from, until int32, values map[MetricRequest][]*MetricData) ([]*MetricData, error) {
	...
	case "linearRegression": // linearRegression(seriesList, startSourceAt=None, endSourceAt=None)
		...
		if len(e.args) > 2 {
			endShift, err := getIntervalArg(e, 2, -1)
			if err != nil {
				return nil, err
			}
			end = until + endShift
		...
		if len(e.args) > 1 {
			startShift, err := getIntervalArg(e, 1, -1)
			if err != nil {
				return nil, err
			}
			start = until + startShift
		}
		// TODO check work with different from/until params

		var results []*MetricData

		for _, a := range arg {
			r := *a
			r.Name = fmt.Sprintf("linearRegression(%s, %d, %d)", a.GetName(), start, end)
			fmt.Printf("linearRegression(%s, %d, %d)\n", a.GetName(), start, end)

			lrV := make([]float64, 0)
			lrI := make([]float64, 0)
			curr := from
			for i := range a.Values {
				if (curr >= start && curr <= end) && !a.IsAbsent[i] {
					lrV = append(lrV, a.Values[i])
					lrI = append(lrI, float64(i))
				}
				curr += a.StepTime
			}
			alpha, beta := stat.LinearRegression(lrI, lrV, nil, false)
			r.IsAbsent = make([]bool, len(r.Values))
			r.Values = make([]float64, len(a.Values))
			for i := range a.Values {
				r.Values[i] = alpha + float64(i)*beta
			}
			results = append(results, &r)
		}
		return results, nil
